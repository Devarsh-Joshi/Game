...
export default function LandingPage() {
  ...
  const handleCreateRoom = () => {
    setIsCreating(true);
    setError('');
    console.log(`[LANDING] Creating room with roundDuration: ${roundDuration} (type: ${typeof roundDuration})`);

    socket.emit('create-room', { totalRounds: Number(totalRounds), roundDuration: Number(roundDuration) }, (response) => {
      setIsCreating(false);
      if (response.success) {
        localStorage.setItem('roomId', response.roomCode);
        localStorage.setItem('playerId', response.playerId);
        localStorage.setItem('isHost', 'true');
        localStorage.setItem('roundDuration', String(response.roundDuration || roundDuration));
        localStorage.setItem('totalRounds', String(response.totalRounds || totalRounds));
        ...
        <div className="game-panel p-6 md:p-8 flex flex-col justify-center text-center h-full min-h-[350px] border-[var(--primary)] shadow-[8px_8px_0_#ff007f]">
          ...
          <div className="mb-6 flex flex-col items-center">
            ...
            <div className="w-full relative mb-2">
              <select
                value={isCustom ? 'custom' : roundDuration}
                onChange={(e) => {
                  const val = e.target.value;
                  if (val === 'custom') {
                    setIsCustom(true);
                    setRoundDuration(customInputValue);
                  } else {
                    setIsCustom(false);
                    setRoundDuration(Number(val));
                  }
                }}
              ...
              <div className="mb-6">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setCustomInputValue(clamped);
                    setRoundDuration(clamped);
                  }}
                  className="w-full bg-[#150722] text-[var(--accent)] font-black text-2xl text-center p-4 rounded-2xl border-4 border-[var(--secondary)] shadow-inner focus:outline-none focus:ring-2 focus:ring-[var(--primary)]"
                />
                <p className="text-xs text-slate-400 text-center mt-1">Enter a value between 5 and 120 seconds</p>
              </div>
            )}
          </div>
